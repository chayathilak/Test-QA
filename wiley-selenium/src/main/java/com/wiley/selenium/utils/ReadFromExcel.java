package com.wiley.selenium.utils;
			...
			for (int colIndex = 0; cellIterator.hasNext(); colIndex++) {
				...
				case Cell.CELL_TYPE_NUMERIC:
					...
					if (DateUtil.isCellDateFormatted(cell)) {
						...
						valueAsSeenInExcel = dateFormat.format(cell.getDateCellValue()) + "\t\t";
					} else {
						DataFormatter fmt = new DataFormatter();
						valueAsSeenInExcel = fmt.formatCellValue(cell);
					}

					tabArray[rowIndex][colIndex] = valueAsSeenInExcel;
					break;
				case Cell.CELL_TYPE_BOOLEAN:
					tabArray[rowIndex][colIndex] = String.valueOf(cell.getBooleanCellValue());
					break;
				default:
